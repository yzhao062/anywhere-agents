...
The visible v0.6.1 theme is **portable statusLine for Claude Max + Codex quota**. Bootstrap now deploys `scripts/statusline.py` to `~/.claude/statusline.py` and merges a matching `statusLine` block into `~/.claude/settings.json` so every consumer project gets an always-on quota readout in the Claude Code terminal status row without per-machine setup.

### Added

- **Statusline renderer (`scripts/statusline.py`)**: A Python script that reads the `rate_limits` field Claude Code v2.1.80+ injects into statusLine stdin JSON (Pro/Max subscribers only) and shows the 5-hour and weekly quota remaining plus reset countdown. Tails the most recent `~/.codex/sessions/YYYY/MM/DD/rollout-*.jsonl` for Codex's `payload.rate_limits` (primary=5h, secondary=7d) and renders Codex quota inline. Windows that have already reset are flagged `(stale)`; a missing `rate_limits` block (API-key sessions, or the first few seconds before the first API response) renders as em-dash. Cross-platform: tested byte-identical output on Windows 11 (Git Bash + Miniforge Python) and Ubuntu 24.04 ARM64 (DGX Spark, ~194ms per render). The Codex section omits cleanly when no `~/.codex/sessions/` is present.
- **Bootstrap deploy + settings entry**: Both `bootstrap.sh` and `bootstrap.ps1` gain a small deploy block (mirror of the `guard.py` deploy pattern) copying `scripts/statusline.py` into `~/.claude/statusline.py`. `user/settings.json` gains a `statusLine` block: `{"type": "command", "command": "\"$HOME/.claude/hooks/_python\" \"$HOME/.claude/statusline.py\""}`. The bootstrap user-settings merger lands the command on every consumer's `~/.claude/settings.json` without overwriting unrelated keys. Uses the existing `_python` wrapper so the Windows Store python shim does not capture the invocation. `scripts/check-parity.sh` adds `scripts/statusline.py` to the STRICT byte-identical list alongside `guard.py` and `session_bootstrap.py`, so future ac/aa drift fails the release-gate. `scripts/remote-smoke.sh` adds a `User-level statusLine deployed` step that asserts the renderer file, the `statusLine` entry in the merged `~/.claude/settings.json`, AND a mtime gate that fails if `~/.claude/settings.json` was not rewritten by the install (so a release-gate machine that reused `$HOME` from a prior smoke run cannot silently pass on a stale entry). `scripts/check-parity.sh` auto-detects when invoked from an `anywhere-agents` checkout with a sibling `agent-config/` and swaps roots accordingly, so the script does not silently self-compare when run from aa.
