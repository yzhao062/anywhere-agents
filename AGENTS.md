...
- **Recommended Codex defaults** (added to `~/.codex/config.toml` on POSIX or `%USERPROFILE%\.codex\config.toml` on Windows; the MCP server reads the same file as interactive sessions):
  ...
  **Default to `fast`; dial down to `standard` only for an unusually large hands-on task.** `fast` bills 2.5x on Astra and the 5.6 family, which is worth it when a human is waiting on the tokens and the account has quota to spare (for example a second Codex account absorbs the rate). Here the latency saved is worth more than the extra credits. For a rare large task where the 2.5x would bite, dial down with a `standard` V2 profile (`~/.codex/std.config.toml`, selected by `codex -p std`) or `/fast off` mid-session. The `implement-review` Codex dispatcher stays on standard independently of this policy: it passes `--ignore-user-config` to `codex exec` for MCP isolation (agent-config#1), so the configured tier does not reach it; `CODEX_DISPATCH_ISOLATE_MCP=off` lifts the isolation and restores the full user config, tier included. Current `prun` routing never selects its legacy Codex dispatcher. The fast default therefore governs interactive and MCP sessions, exactly where the latency is worth paying for. Hardcoding `fast` into the isolated review path is deliberately avoided, because it would fail every round for a consumer whose account lacks the tier. `fast` is the current config spelling and maps to the request value `priority`; a config already reading `priority` is the same tier.
