...
| `prun` | explicit parallel delegation / fan-out intent; not auto-routed by file type | Fan out independent task units to Sonnet and Agy workers while the Opus session coordinates; Codex is reserved for `/vet`, not fan-out; workers spend separate quotas and never commit or push |
