...
def _composer_source_root() -> Path:
    """Return the source tree that owns this composer script.

    When run from a bootstrapped project this is
    ``<project>/.agent-config/repo``. When run from the PyPI wheel's
    bundled composer, this is ``anywhere_agents/composer``. Bundled pack
    manifests and file sources should come from this tree, not from a
    possibly stale project-local composer.
    """
    return Path(__file__).resolve().parent.parent


def _is_packaged_composer() -> bool:
    """Return True when running from ``anywhere_agents/composer``."""
    source_root = _composer_source_root()
    return source_root.name == "composer" and source_root.parent.name == "anywhere_agents"
...
def _resolve_manifest_path(root: Path, explicit: Path | None) -> Path:
    ...
    project_candidates = (
        ...
        bootstrap_dir / "rule-packs.yaml",
    )
    source_root = _composer_source_root()
    source_candidates = (
        source_root / "bootstrap" / "packs.yaml",
        source_root / "bootstrap" / "rule-packs.yaml",
    )
    candidates = (
        source_candidates + project_candidates
        if _is_packaged_composer()
        else project_candidates + source_candidates
    )
...
def _build_ctx(
    *,
    ...
    if archive is not None:
        ...
        # packs use the source tree that owns this composer. That matters
        # for the PyPI CLI's bundled composer: the project-local
        # `.agent-config/repo/` may be stale, while the installed package
        # carries the current aa-core-skills content.
        pack_source_dir = (
            archive_dir if archive_dir is not None
            else (
                _composer_source_root()
                if _is_packaged_composer()
