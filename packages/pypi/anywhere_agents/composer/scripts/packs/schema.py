...
``ParseError`` when the manifest contains:

- A private ``source:`` URL (SSH, ``git@``, explicit ``auth:`` field) on a
  v2-shape pack entry. Per pack-architecture.md, schema-layer rejection in
...
# parse time on manifest packs as defense in depth; the v0.5.0 auth chain
# handles private fetch via inline-selection sources at the compose layer.
...
def parse_manifest(path: Path) -> dict[str, Any]:
    ...
    ParseError
        ...
        contains rejected forms (private sources, unknown kinds, or
        unknown ``update_policy`` values).
    """
    if not path.exists():
...
def _validate_v2_pack(path: Path, idx: int, entry: dict[str, Any]) -> None:
    ...
    # Normalize the resolved policy back into the parsed pack so downstream
    # consumers (compose_packs, dispatch) don't re-default to "locked"
    # when reading an in-memory pack dict that omitted the field. v0.5.0
    # default flipped from "locked" to "prompt" (Codex Round 2 H1).
    entry["update_policy"] = update_policy
...
def _reject_private_url(
    ...
    """Reject private-source URL on a manifest pack at parse time.

    Defense in depth at the schema layer: manifest-declared packs must
    use anonymous HTTPS URLs. Private fetch (SSH, ``git@``, explicit
    auth URLs) is handled by the v0.5.0 auth chain via inline-selection
    sources at the compose layer, not via manifest entries.
    """
